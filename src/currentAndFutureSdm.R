...
#get occurence data, just lat/long
...
#create the raster stack form the files list 
currentClimRasterStack <- raster::stack(climList)

# test run (annual temp)
plot(currentClimRasterStack[[1]]) 

# plot occurrence points on annual temp map 
...
# drill down on where data lives (the US)
geographicExtent <- extent(x = occurrenceSpatialPts)


# standardize points for reproducability 
set.seed(45) 
...
# create a grid of climate measurements per occurrence points 
occEnv <- na.omit(raster::extract(x = currentClimRasterStack, y = occurrenceCoords))

# create a gird of climate measurements for pseudo-absence points 
absenceEnv<- na.omit(raster::extract(x = currentClimRasterStack, y = backgroundPoints))


# create a vector of presence and absence 
presenceAbsenceV <- c(rep(1, nrow(occEnv)), rep(0, nrow(absenceEnv))) 

# create a data frame of presence/absence data by climate variable 
